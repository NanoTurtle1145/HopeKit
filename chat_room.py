...
class WebWindow(QMainWindow):
    def __init__(self):
        ...
        self.web_view.load(QUrl("https://hopestudio.top/"))
    ...
    def _on_load_finished(self, ok: bool):
        if ok:
            ...
            self.web_view.setHtml(
                ...
                "<p>网站 <b>hopestudio.top</b> 当前无法访问。</p>"
                "<p>可能原因：服务器故障 (502 Bad Gateway) 或网络连接问题。</p>"
...
class OtherWindow(QMainWindow):
    def __init__(self):
        ...
        self.logbutton = QPushButton('开发日志', central)
        self.webbutton = QPushButton('简易官网', central)
        self.webbutton.clicked.connect(self._open_web)
        self.logbutton.clicked.connect(self._open_log)
    ...
    def _open_web(self):
        # 优先尝试内置 WebEngine 浏览器；失败则回退到系统默认浏览器
        try:
...
# ============================================================
#  ServerThread — 服务端监听线程
# ============================================================
class ServerThread(QThread):
    _signal = pyqtSignal(str)   # 状态消息
    _text = pyqtSignal(str)     # 接收到的聊天文本
    _flag = pyqtSignal(str)     # 连接状态标志 (connect / disconnect)
...
class MainWin(QMainWindow):
    ...
    def _create_widgets(self):
        ...
        self.otherBtn = QPushButton("其他")
        self.otherBtn.clicked.connect(self._open_other)
    ...
    def _compose_layout(self):
        ...
        control_layout.addWidget(self.otherBtn)
    ...
    def _open_other(self):
        if self._other_win is None:
            self._other_win = OtherWindow()
        self._other_win.show()
